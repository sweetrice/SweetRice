 ...
 */
	...
	$where = " (ps.`title` LIKE '%$tag%' OR ps.`body` LIKE '%$tag%' OR ps.`tags` LIKE '%$tag%') AND ps.`in_blog` = '1' AND ip.`item_type` = 'post' ";
	...
	if($m=='pins'){
		...
		$data = db_fetch(array(
			...
			'field' => "ps.`id`,ps.`sys_name`,ps.`category`,ps.`name`,ps.`body`,ps.`views`,ps.`date`,ps.`tags`,ps.`id`",
