...
echo '<title><![CDATA[', htmlspecialchars_decode($row['name'], ENT_QUOTES), ']]></title>', "\n<image><title><![CDATA[", htmlspecialchars_decode($row['name'], ENT_QUOTES), "]]></title><url>" . ($global_setting['logo'] ? SITE_URL . ATTACHMENT_DIR . $global_setting['logo'] : BASE_URL . 'images/sweetrice.png') . "</url><link><![CDATA[", htmlspecialchars_decode($row['name'], ENT_QUOTES), "]]></link></image>\n<link><![CDATA[" . BASE_URL . show_link_page($categories[$row['category']]['link'], $row['sys_name']) . "]]></link>\n", "<description><![CDATA[", htmlspecialchars_decode($row['description'], ENT_QUOTES), "]]></description>", "\n";
...
foreach ($comments as $comment) {
    $no += 1;
    if ($no == 1) {
        $comment_body .= '<h2>' . _t('User Comment') . '</h2>';
    }
