...
<thead>
	<tr>
		...
		<th class="max50"><a href="javascript:void(0);" class="btn_sort" data="name"><?php _e('Name');?></a></th>
		<th><a href="javascript:void(0);" class="btn_sort" data="slug"><?php _e('Slug');?></a></th>
		<th><a href="javascript:void(0);" class="btn_sort" data="parent"><?php _e('Parent');?></a></th>
		<th><a href="javascript:void(0);" class="btn_sort" data="title"><?php _e('Title');?></a></th>
...
<tr class="<?php echo $classname; ?>" id="tr_<?php echo $no; ?>">
	...
	<td class="max50" data-label="<?php _e('Name');?>"><a href="<?php echo BASE_URL . show_link_cat($row['link'], ''); ?>" target="_blank"><span id="name_<?php echo $no; ?>"><?php echo $row['name']; ?></span></a></td>
	<td data-label="<?php _e('Slug');?>"><span id="slug_<?php echo $no; ?>"><?php echo $row['link']; ?></span></td>
	<td data-label="<?php _e('Parent');?>"><span id="parent_<?php echo $no; ?>"><a href="<?php echo show_link_cat($categories[$row['parent_id']]['link']); ?>"><?php echo $row['parent_id'] ? $categories[$row['parent_id']]['name'] : _t('Main'); ?></a></span></td>
	<td data-label="<?php _e('Title');?>"><span id="title_<?php echo $no; ?>"><?php echo $row['title']; ?></span></td>
	<td class="td_admin" data-label="<?php _e('Admin');?>"><span id="action_<?php echo $no; ?>"></span><a title="<?php _e('Delete');?>" class="action_delete" data="<?php echo $row['id']; ?>" no="<?php echo $no; ?>" href="javascript:void(0);"><?php _e('Delete');?></a> <a title="<?php _e('Modify');?>" class="action_modify" href="./?type=category&mode=insert&id=<?php echo $row['id']; ?>"><?php _e('Modify');?></a></td>
</tr>
<?php
}
?>
