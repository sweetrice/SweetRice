		 ...
		 if($entry != '.' && $entry != '..'){
			 if($keyword){
				 if(strpos($entry,$keyword)!==false){
					$tmp = array('name'=>$entry,'type'=>(is_dir($_open_dir.$entry)?'dir':sr_file_type($_open_dir.$entry)),'date'=>filectime($_open_dir.$entry),'link'=>$open_dir.$entry);
					$files[]  = $tmp;						
				 }
			 }else{
				$tmp = array('name'=>$entry,'type'=>(is_dir($_open_dir.$entry)?'dir':sr_file_type($_open_dir.$entry)),'date'=>filectime($_open_dir.$entry),'link'=>$open_dir.$entry);
