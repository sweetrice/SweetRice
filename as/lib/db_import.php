...
if (is_dir($db_backup_dir)) {
	...
	<tr id="tr_<?php echo $no; ?>">
		<td><span class="sortNo" id="sortNo_<?php echo $no; ?>"><?php echo $no; ?></span><input type="checkbox" name="plist[]" class="ck_item" value="<?php echo $entry; ?>"/></td>
		<td class="max50" data-label="<?php _e('Name');?>"><a href="javascript:void(0);" url="./?type=data&mode=db_import&db_file=<?php echo $entry; ?>&form_mode=import" class="btn_import"><span id="name_<?php echo $no; ?>"><?php echo $entry; ?></span></a> (<?php echo number_format(filesize($db_backup_dir . '/' . $entry)); ?> bytes)</td>
		<td class="td_admin" data-label="<?php _e('Admin');?>"><span id="action_<?php echo $no; ?>"></span><a title="<?php _e('Delete');?>" class="action_delete" data="<?php echo $entry; ?>" no="<?php echo $no; ?>" href="javascript:void(0);"><?php _e('Delete');?></a><a title="<?php _e('Save');?>" class="action_save" href="./?type=data&mode=db_import&db_file=<?php echo $entry; ?>&form_mode=save"><?php _e('Save');?></a></td>
	</tr>
<?php
}
    }
