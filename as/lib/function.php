...
$dashboard_acts = dashboard_acts();
	...
	<div><?php echo $val['title']; ?>
	<div class="hidden_ pl10">
	<?php if (is_array($val['child']) && count($val['child']) > 0) {foreach ($val['child'] as $v): if (isset($v['mustBase']) && !dashboard_role($v['request']['type'], $v['mustBase'])) {continue;}
                $str = '';foreach ($v['request'] as $kk => $vv) {$str .= '&' . $kk . '=' . $vv;}?>
	...
	<div class="hidden_ pl10">
	<?php foreach ($val['child'] as $v): if (isset($v['mustBase']) && !dashboard_role($v['name'], $v['mustBase'])) {continue;}
            $str = '';foreach ($v['request'] as $kk => $vv) {$str .= '&' . $kk . '=' . $vv;}?>
